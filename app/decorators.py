...
def requires_role(*roles: UserRole):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Pega o cargo do usuário como texto puro, independente de como o banco entrega
            user_role_str = getattr(current_user.role, 'name', str(current_user.role)).upper()
            
            # Pega os cargos permitidos da rota
            allowed_roles_str = [getattr(r, 'name', str(r)).upper() for r in roles]
