...
class ActivityType(db.Model):
    ...
    # Valor padrão de Premiles por atividade (editável pelo supervisor)
...
class ChecklistLog(db.Model):
    ...
    junior = db.relationship("User", back_populates="checklist_logs", foreign_keys=[junior_id])
...
class ManualLog(db.Model):
    ...
    junior = db.relationship("User", back_populates="manual_logs", foreign_keys=[junior_id])
...
class PremilesBalance(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    junior_id = db.Column(db.Integer, db.ForeignKey("users.id"), unique=True, nullable=False)
